...
def build_strands_tools(service):
    ...
    @tool
    def inspect_recorded_events(item_id: str) -> list[dict]:
        """Inspect recorded event history for an item without changing it."""
        return [asdict(event) for event in service.workflow.state.events if event.item_id == item_id]

    @tool
    def request_evidence(item_id: str) -> dict:
        """Request evidence through the provider-neutral application service."""
        return asdict(service.request_evidence(item_id, dispatch=False))
    ...
    def request_human_review(item_id: str) -> dict:
        ...
        return asdict(service.request_human_review(item_id, dispatch=False))

    return [inspect_clearance_item, inspect_evidence, inspect_recorded_events, request_evidence, process_incoming_document, request_human_review]
