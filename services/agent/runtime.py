...
class ClearanceApplicationService:
    ...
    def request_evidence(self, item_id: str, dispatch: bool = True):
        if dispatch:
            self._dispatch("REQUEST_EVIDENCE", f"request-{item_id}", item_id, "No evidence record was found.")
    ...
    def request_human_review(self, item_id: str, dispatch: bool = True):
        if dispatch:
            self._dispatch("REQUEST_HUMAN_REVIEW", f"review-{item_id}", item_id, "Evidence cannot establish administrative permission scope.")
